...
<!-- LOCAL MODIFICATION: this file diverges from the upstream kunchenguid/lavish-axi source tracked in skills-lock.json. See skills-lock.json's localModifications for what changed and why before re-syncing from upstream. -->
...
lavish-axi is installed globally on this machine - invoke it directly with `lavish-axi <html-file>`, not through `npx`.
If lavish-axi output shows a follow-up command starting with `lavish-axi`, run it exactly as shown, without prefixing `npx -y`.
...
1. Run `lavish-axi` with no arguments first - it shows any currently open sessions (across all projects; state is global under `~/.lavish-axi/`) plus the latest usage guidance for the installed version, so check it before assuming there's no session already open for this artifact.
2. Create the HTML artifact (default location `.lavish/<name>.html` in the working directory).
3. Run `lavish-axi <html-file>` to open or resume a review session in the browser.
4. Run `lavish-axi poll <html-file>` to long-poll for the user's annotations, queued prompts, and browser-reported `layout_warnings`.
...
5. If poll returns `layout_warnings`, follow the returned `next_step`: fix and re-check fresh error-severity findings, but proceed with a note instead of looping when every current warning is persistent or low-severity.
6. Apply human feedback, then poll again with `--agent-reply "<message>"` to reply in the browser and keep the loop going.
7. Run `lavish-axi end <html-file>` when the review is finished.
8. If the user ends the session from the browser instead, `lavish-axi <html-file>` refuses to reopen it and says so - only pass `--reopen` when the user asks for further review or something genuinely important needs their visual attention. Otherwise deliver remaining updates directly in this conversation.
